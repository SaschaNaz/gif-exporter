...
namespace GIFExporter {
    ...
    interface Frame { data: ImageData, delay: number }
    export interface FrameOffset { x: number, y: number }
    ...
    export const SuperGif = (opts: Options) => {
        ...
        const handler = {
            ...
            app: {
                ...
                NETSCAPE: withProgress<Block>(doNothing)
            },
        ...
        const instance = {
            ...
            load_raw: (arr: Uint8Array, callback: Callback) => {
                if (!load_setup(callback)) return;
                if (!initialized) init();
